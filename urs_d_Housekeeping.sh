...
for i in $(ls $DAT_DIR | grep .*"\.dat"$); do           ## check every .dat in dat dir
        ...
        if [ $(stat -c %Y $DAT_DIR/$i) -ge $(cat $TSTAMP_DIR/urs_d_TransferFinAndDat.txt | grep $i$ | cut -f2 -d" ") ]; then ## if current timestamp is greater than equal to listed timestamp then delete the file
                echo $(date +'%Y%m%d')"|$DAT_DIR/$i exceeded its lifetime and is now being deleted" >> $LOG_DIR/urs_d_Housekeeping_$(date +'%Y%m%d').log;
                sed -i "/$i$/d" $TSTAMP_DIR/urs_d_TransferFinAndDat.txt; ## remove entry in timestamp file
                rm $DAT_DIR/$i;
        fi;
done
for i in $(ls $FIN_DIR | grep .*"\.dat\.FIN"$); do      ## check every .dat.FIN in fin dir
        touch -m $FIN_DIR/$i                            ## update modify timestamp
        if [ $(stat -c %Y $FIN_DIR/$i) -ge $(cat $TSTAMP_DIR/urs_d_TransferFinAndDat.txt | grep $i$ | cut -f2 -d" ") ]; then ## if current timestamp is greater than equal to listed timestamp then delete the file
...
for i in $(ls $USAGE_DIR | grep .*"\.dat".*$); do       ## check every .dat and .dat.FIN in usage dir
        ...
        if [ $(stat -c %Y $USAGE_DIR/$i) -ge $(cat $TSTAMP_DIR/urs_d_TransferFinAndDat.txt | grep $i$ | cut -f2 -d" ") ]; then ## if current timestamp is greater than equal to listed timestamp then delete the file
